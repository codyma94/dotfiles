...
/*
   This file contains the implementation of the local tapestry node.

   There are three kinds of methods defined in this file:
       ...
       3.  Common utility methods that you can use in your implementation (e.g. findRoot)
...
func (local *TapestryNode) Join(otherNode Node) error {
	...
	neighbors, err := AddNode_RPC(root, local.node)
	if err != nil {
...
// Client API.  Publishes the key in tapestry.
//
...
//	* Fetch the replicas (nodes storing the blob) from the root's object store
...
*/
func (local *TapestryNode) AddNode(node Node) (neighborset []Node, err error) {
...
*/
func (local *TapestryNode) AddNodeMulticast(newnode Node, level int) (neighbors []Node, err error) {
