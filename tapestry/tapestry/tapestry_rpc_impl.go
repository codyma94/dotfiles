...
func (server *TapestryRPCServer) AddNode(req NodeRequest, rsp *[]Node) (err error) {
	...
	neighbors, err := server.tapestry.local.AddNode(req.Node)
	*rsp = append(*rsp, neighbors...)
	return
}
...
func (server *TapestryRPCServer) AddNodeMulticast(req AddNodeMulticastRequest, rsp *[]Node) (err error) {
	...
	neighbors, err := server.tapestry.local.AddNodeMulticast(req.NewNode, req.Level)
	*rsp = append(*rsp, neighbors...)
	return err
}
